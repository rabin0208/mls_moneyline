"""
Fit logistic regression to predict home_win.
...
def main() -> None:
    ...
    print(f"  Train home rate: {y_train.mean():.3f}  test home rate: {y_test.mean():.3f}")

    scaler = StandardScaler()
    X_train_s = scaler.fit_transform(X_train)
    X_test_s = scaler.transform(X_test)

    print("\nFitting logistic regression (predict home_win)...")
    ...
    joblib.dump(model, MODELS_DIR / "home_logistic.pkl")
    joblib.dump(scaler, MODELS_DIR / "home_scaler.pkl")
    print(f"\nSaved model and scaler to {MODELS_DIR}/")

    coef = pd.DataFrame(
        {"feature": FEATURE_COLS, "coefficient": model.coef_[0]}
    ).sort_values("coefficient", key=abs, ascending=False)
    coef_path = TABLES_DIR / "home_logistic_coefficients.csv"
